...
__global__ void qwen35_attention_v1_kernel(const __nv_bfloat16 *__restrict__ query,
    ...
    for (int key_token = 0; key_token < visible_key_tokens; ++key_token) {
        ...
        for (int feature = 0; feature < QWEN35_HEAD_DIM; ++feature) {
            output_accumulator[feature] =
                output_accumulator[feature] * previous_scale +
    ...
    for (int feature = 0; feature < QWEN35_HEAD_DIM; ++feature) {
        output[output_offset + feature] =
            __float2bfloat16(output_accumulator[feature] / running_sum);
    }
}
...
torch::Tensor attention_cuda(torch::Tensor query, torch::Tensor key, torch::Tensor value,
    ...
    if (version == 1) {
        ...
        const Qwen35AttentionParams attention_params{
            query_ptr, key_ptr, value_ptr, output_ptr, batch_size, query_tokens, key_tokens};
        ...
        const Qwen35AttentionParams params{
            query_ptr,  key_ptr,      value_ptr, head_major_output_ptr,
            batch_size, query_tokens, key_tokens};
        launch_qwen35_attention_v6(params, stream);
        output = head_major_output.permute({0, 2, 1, 3}).contiguous();
    } else {
        TORCH_CHECK(false, "attention version must be 1, 2, 3, 4, 5, or 6");
    }
